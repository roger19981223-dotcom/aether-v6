...
const serverHTML = `<!DOCTYPE html><html lang="zh-CN"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Aether Server Panel</title><script src="https://cdn.tailwindcss.com"></script><script src="https://unpkg.com/vue@3/dist/vue.global.prod.js"></script><style>body{background-color:#f9fafb;color:#111827;font-family:system-ui,-apple-system,sans-serif} .ring{border-radius:50%;display:flex;align-items:center;justify-content:center;box-shadow:inset 0 0 10px rgba(0,0,0,0.05)} .flex{display:flex} .flex-col{flex-direction:column} .items-center{align-items:center} .justify-center{justify-content:center} .text-center{text-align:center} .w-full{width:100%} .mb-4{margin-bottom:1rem}</style></head><body><div id="app" class="min-h-screen flex"><div v-if="!log" class="m-auto bg-white p-8 rounded-xl shadow-2xl w-96 border border-gray-100"><h2 class="text-2xl font-black mb-8 text-center tracking-tight">Aether /<span class="text-gray-400">Admin</span></h2><div class="space-y-4"><input v-model="u" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black transition" placeholder="管理员账号"><input v-model="p" type="password" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black transition" placeholder="管理密码" @keyup.enter="li"><button @click="li" class="w-full bg-black text-white p-3 rounded font-bold hover:bg-gray-800 transition shadow-lg">登录面板</button></div></div><template v-else><div class="w-64 bg-black text-white p-6 flex flex-col shadow-2xl z-10" style="min-width:256px"><div class="text-2xl font-black mb-10 tracking-tighter">Aether<span class="text-blue-500 text-3xl leading-none">.</span></div><div class="space-y-2 flex-1"><div @click="t='s'" :class="t=='s'?'bg-gray-800 text-white':'text-gray-400 hover:text-white hover:bg-gray-900'" class="p-3 cursor-pointer rounded transition font-medium">系统状态 (Status)</div><div @click="fu();t='u'" :class="t=='u'?'bg-gray-800 text-white':'text-gray-400 hover:text-white hover:bg-gray-900'" class="p-3 cursor-pointer rounded transition font-medium">用户管理 (Users)</div><div @click="fc();t='c'" :class="t=='c'?'bg-gray-800 text-white':'text-gray-400 hover:text-white hover:bg-gray-900'" class="p-3 cursor-pointer rounded transition font-medium">面板设置 (Settings)</div></div><div class="text-xs text-gray-600 mt-auto">Aether Core Enterprise</div></div><div class="flex-1 p-10 overflow-y-auto" style="width:100%"><div v-if="t=='s'" class="space-y-8"><h2 class="text-3xl font-bold mb-6 tracking-tight">系统状态</h2><div class="bg-white p-8 shadow-sm rounded-2xl border border-gray-100 flex justify-around items-center text-center flex-wrap"><div class="flex flex-col items-center"><div class="w-32 h-32 ring mb-4" style="width:128px;height:128px;background:conic-gradient(#3b82f6 30%, #f3f4f6 0)"><div class="w-28 h-28 bg-white rounded-full flex flex-col items-center justify-center" style="width:112px;height:112px;background-color:white;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;"><span class="text-xl font-bold text-gray-800">{{st.cpu}}</span></div></div><div class="text-sm font-bold text-gray-500 tracking-wider">系统核心 (CPU)</div></div><div class="flex flex-col items-center"><div class="w-32 h-32 ring mb-4" style="width:128px;height:128px;background:conic-gradient(#10b981 40%, #f3f4f6 0)"><div class="w-28 h-28 bg-white rounded-full flex flex-col items-center justify-center" style="width:112px;height:112px;background-color:white;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;"><span class="text-xl font-bold text-gray-800">{{st.mem}}</span></div></div><div class="text-sm font-bold text-gray-500 tracking-wider">分配内存 (RAM)</div></div><div class="flex flex-col items-center"><div class="w-32 h-32 ring mb-4" style="width:128px;height:128px;background:conic-gradient(#8b5cf6 60%, #f3f4f6 0)"><div class="w-28 h-28 bg-white rounded-full flex flex-col items-center justify-center" style="width:112px;height:112px;background-color:white;border-radius:50%;display:flex;flex-direction:column;align-items:center;justify-content:center;"><span class="text-xl font-bold text-gray-800">{{st.go}}</span></div></div><div class="text-sm font-bold text-gray-500 tracking-wider">并发协程 (Routines)</div></div></div><div class="grid grid-cols-3 gap-6" style="display:flex;flex-wrap:wrap;gap:1.5rem"><div class="bg-white p-6 shadow-sm rounded-2xl border border-gray-100" style="flex:1;min-width:200px"><div class="text-sm font-bold text-gray-400 mb-1">系统运行时间</div><div class="text-2xl font-black text-gray-800">{{st.ut}}</div></div><div class="bg-white p-6 shadow-sm rounded-2xl border border-gray-100" style="flex:1;min-width:200px"><div class="text-sm font-bold text-gray-400 mb-1">上行总流量 (TX)</div><div class="text-2xl font-black text-blue-600">{{st.tx}}</div></div><div class="bg-white p-6 shadow-sm rounded-2xl border border-gray-100" style="flex:1;min-width:200px"><div class="text-sm font-bold text-gray-400 mb-1">下行总流量 (RX)</div><div class="text-2xl font-black text-green-600">{{st.rx}}</div></div></div></div><div v-if="t=='u'"><div class="flex justify-between items-center mb-6"><h2 class="text-3xl font-bold tracking-tight">客户接入管理</h2><button @click="sa=true;isEdit=false;nu={enable:true,LimitGb:0,ExpStr:''}" class="bg-black text-white px-6 py-2 rounded-lg font-bold shadow-lg hover:bg-gray-800 transition">+ 添加客户</button></div><div class="bg-white shadow-sm rounded-2xl border border-gray-100 overflow-hidden"><table class="w-full text-left text-sm"><thead class="bg-gray-50 text-gray-500 border-b border-gray-100"><tr><th class="p-4 font-bold">状态</th><th class="p-4 font-bold">用户名</th><th class="p-4 font-bold">到期日</th><th class="p-4 font-bold">流量配额</th><th class="p-4 font-bold">TX / RX</th><th class="p-4 font-bold text-right">操作</th></tr></thead><tbody><tr v-for="u in us" class="border-b border-gray-50 hover:bg-gray-50 transition"><td class="p-4"><span v-if="u.enable" class="px-2 py-1 bg-green-100 text-green-700 rounded text-xs font-bold">活动</span><span v-else class="px-2 py-1 bg-red-100 text-red-700 rounded text-xs font-bold">封禁</span></td><td class="p-4 font-bold text-gray-800">{{u.username}}</td><td class="p-4 text-gray-600">{{u.Exp}}</td><td class="p-4 text-gray-600"><span v-if="u.limit>0">{{(u.limit/1073741824).toFixed(2)}} GB</span><span v-else class="text-gray-400">无限制</span></td><td class="p-4 font-mono text-gray-500 text-xs">{{u.Tx}} / {{u.Rx}}</td><td class="p-4 text-right"><button @click="eu(u)" class="text-blue-500 hover:text-blue-700 font-bold transition mr-4">编辑</button><button @click="du(u.id)" class="text-red-500 hover:text-red-700 font-bold transition">删除</button></td></tr></tbody></table></div></div><div v-if="t=='c'" class="space-y-6 max-w-3xl"><h2 class="text-3xl font-bold tracking-tight mb-6">节点核心配置</h2><div class="bg-white p-8 shadow-sm rounded-2xl border border-gray-100 space-y-5"><div class="grid grid-cols-2 gap-6" style="display:flex;gap:1.5rem"><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">面板登录账号</label><input v-model="cf.panel_user" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">面板登录密码</label><input v-model="cf.panel_pass" type="password" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div></div><div><label class="block text-sm font-bold text-gray-500 mb-2">服务器绑定域名 (留空则自签证书，填入自动申请 Let's Encrypt)</label><input v-model="cf.domain" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black" placeholder="aether.yourdomain.com"></div><div class="grid grid-cols-2 gap-6" style="display:flex;gap:1.5rem"><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">核心监听端口 (逗号分隔)</label><input v-model="cf.listen_ports" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div style="flex:1"><label class="block text-sm font-bold text-gray-500 mb-2">伪装站策略</label><select v-model="cf.camo_mode" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"><option value="proxy">一键反向代理 (支持 REALITY 无感转发)</option><option value="self">自己搭建 (内置纯净 Nginx 欢迎页)</option><option value="local">本地防探测 (返回 400 Bad Request)</option></select></div></div><div v-if="cf.camo_mode=='proxy'"><label class="block text-sm font-bold text-gray-500 mb-2">反向代理/REALITY 目标地址 (默认推荐 www.microsoft.com:443)</label><input v-model="cf.camo_target" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black" placeholder="1.1.1.1:80"></div><div><label class="block text-sm font-bold text-gray-500 mb-2">机场对接 API Token</label><input v-model="cf.api_token" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black font-mono text-sm" placeholder="预共享鉴权密钥"></div><button @click="sc" class="bg-black text-white px-8 py-3 rounded-lg font-bold shadow-lg hover:bg-gray-800 transition w-full mt-4">保存并应用配置</button></div></div></div><div v-if="sa" class="fixed inset-0 bg-gray-900/40 backdrop-blur-sm flex items-center justify-center z-50" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.4);display:flex;align-items:center;justify-content:center"><div class="bg-white p-8 rounded-2xl w-[400px] shadow-2xl border border-gray-100" style="width:400px"><h3 class="text-2xl font-bold mb-6 tracking-tight">{{isEdit?'编辑客户':'添加客户'}}</h3><div class="space-y-4"><div><label class="block text-sm font-bold text-gray-500 mb-1">系统用户名</label><input v-model="nu.username" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div><label class="block text-sm font-bold text-gray-500 mb-1">接入密码</label><input v-model="nu.password" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div><label class="block text-sm font-bold text-gray-500 mb-1">流量配额 (GB，0 为无限制)</label><input v-model.number="nu.LimitGb" type="number" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black"></div><div><label class="block text-sm font-bold text-gray-500 mb-1">过期时间 (留空为永久)</label><input v-model="nu.ExpStr" type="date" class="w-full p-3 bg-gray-50 border border-gray-200 rounded outline-none focus:border-black text-gray-600"></div><div class="flex items-center justify-between mt-4"><span class="font-bold text-gray-700">账户状态</span><button @click="nu.enable=!nu.enable" :class="nu.enable?'bg-green-500':'bg-gray-300'" class="relative inline-flex h-6 w-11 items-center rounded-full transition-colors" style="width:44px;height:24px;border-radius:12px;border:none;cursor:pointer"><span :class="nu.enable?'translate-x-6':'translate-x-1'" class="inline-block h-4 w-4 transform rounded-full bg-white transition" style="display:inline-block;width:16px;height:16px;background:white;border-radius:50%;transition:transform 0.2s;transform:translateX(nu.enable?24px:4px)"></span></button></div><div class="flex gap-4 mt-8" style="display:flex;gap:1rem"><button @click="au" class="flex-1 bg-black text-white p-3 rounded-lg font-bold shadow hover:bg-gray-800 transition" style="flex:1">确认保存</button><button @click="sa=false" class="flex-1 bg-gray-100 text-gray-600 p-3 rounded-lg font-bold hover:bg-gray-200 transition" style="flex:1;background:#f3f4f6">取消</button></div></div></div></div></template></div><script>Vue.createApp({data(){return{log:false,t:'s',sa:false,isEdit:false,u:'',p:'',st:{cpu:'-',mem:'-',go:'-',ut:'-',tx:'-',rx:'-'},us:[],cf:{},nu:{enable:true,LimitGb:0,ExpStr:''}}},methods:{async req(p,m='GET',b){let o={method:m};if(b){o.body=JSON.stringify(b);o.headers={'Content-Type':'application/json'}};try{let r=await fetch('/api/'+p,o);if(r.status===401){this.log=false;throw new Error("401");}return await r.json()}catch(e){throw e;}},async li(){try{let r=await this.req('login','POST',{User:this.u,Pass:this.p});if(r&&r.ok){this.log=true;this.fs()}}catch(e){alert("登录失败: 账号或密码错误");}},async fs(){try{let r=await this.req('status');if(r)this.st=r}catch(e){}},async fu(){try{let r=await this.req('users');if(r)this.us=r}catch(e){}},async fc(){try{let r=await this.req('settings');if(r)this.cf=r}catch(e){}},eu(u){this.nu=JSON.parse(JSON.stringify(u));if(u.expire_at>0){this.nu.ExpStr=new Date(u.expire_at*1000).toISOString().split('T')[0]}else{this.nu.ExpStr=''}this.nu.LimitGb=u.limit/1073741824;this.isEdit=true;this.sa=true},async au(){if(!this.nu.username||!this.nu.password){alert('账号密码必填');return}this.nu.limit=this.nu.LimitGb*1073741824;if(this.nu.ExpStr){this.nu.expire_at=new Date(this.nu.ExpStr).getTime()/1000}else{this.nu.expire_at=0}if(this.isEdit){await this.req('users/edit','POST',this.nu)}else{await this.req('users','POST',this.nu)}this.sa=false;this.fu()},async du(id){if(confirm('警告: 确定删除此客户?')){await this.req('users/del','POST',{ID:id});this.fu()}},async sc(){await this.req('settings','POST',this.cf);alert('保存成功，配置已热更新')}},mounted(){this.req('status').then(r=>{if(r){this.log=true;this.fs();setInterval(()=>{if(this.t=='s')this.fs();if(this.t=='u')this.fu()},2000)}}).catch(e=>{})}}).mount('#app')</script></body></html>`
